  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 md:px-8 py-10">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight text-black">Featured Blogs</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {blogs.map((blog, idx) => (
          ...
          >
            <div className="relative w-full flex items-center justify-center p-4" style={{ background: blog.themeColor, transition: 'background 0.3s' }}>
            ...
            <div className="flex flex-col flex-1 p-5">
              <h3 className="text-xl font-extrabold mb-2 leading-snug text-black">{blog.title.en}</h3>
              <div className="flex items-center text-gray-500 text-sm mb-3 gap-4">
                <span>📅 {blog.date}</span>
                <span>⏱ {getReadTime(blog.content.en)} min read</span>
              </div>
              <p className="text-gray-700 mb-4 line-clamp-2">{blog.description.en}</p>
