...
interface CancerRecommendations {
  "Specially for Cancer": Product[];
  "Gemstone Therapy": CategoryItem[];
  "Crystal Healing": CategoryItem[];
  "Power Jewelry": CategoryItem[];
  "Wellness & Balance": CategoryItem[];
  "Prosperity & Abundance": CategoryItem[];
  "Love & Relationships": CategoryItem[];
  "Personal Growth": CategoryItem[];
}
      ...
      {activeCategory !== 'specially' && (
        ...
        >
          {(() => {
            ...
            const items = (cancerRecommendations as Record<string, unknown[]>)[categoryLabel] as CategoryItem[];
            return items?.map((category: CategoryItem, index: number) => (
              <motion.div
                key={index}
