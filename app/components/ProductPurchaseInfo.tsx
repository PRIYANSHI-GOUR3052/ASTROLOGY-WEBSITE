...
export default function ProductPurchaseInfo() {
  return (
    <section className="w-screen flex flex-col items-center py-8 md:py-12" style={{background:'#F9F6F2', marginLeft:'calc(50% - 50vw)', marginRight:'calc(50% - 50vw)'}}>
      <h2 className="text-xl md:text-2xl lg:text-3xl font-bold mb-6 md:mb-10 text-black text-center tracking-wide uppercase px-4" style={{ fontFamily: 'Playfair Display, serif', letterSpacing: '0.08em' }}>
        BEFORE YOU BUY: OUR PROMISE
      </h2>
      <div className="flex flex-col md:flex-row w-full max-w-none justify-between items-stretch px-4 md:px-8">
        {policies.reduce<React.ReactNode[]>((acc, p, idx) => {
          acc.push(
            <div key={p.title} className="flex flex-col items-center flex-1 min-w-0 px-4 md:px-8 mb-8 md:mb-0">
              <div className="flex flex-col items-center text-center">
                <div className="w-6 h-6 md:w-8 md:h-8">
                  {p.icon}
                </div>
                <div className="mt-2 md:mt-3 text-base md:text-lg font-bold text-black tracking-wide uppercase" style={{ fontFamily: 'Playfair Display, serif', letterSpacing: '0.08em' }}>{p.title}</div>
                <div className="mt-2 mb-4 text-black w-full text-sm md:text-base">{p.desc}</div>
                {p.link && (
                  <Link href={p.link} legacyBehavior>
                    <a className="mt-auto px-4 md:px-6 py-2 border border-black text-black rounded transition hover:bg-black hover:text-white text-xs md:text-sm font-medium" style={{minWidth:100, fontFamily: 'Inter, Montserrat, Playfair Display, Arial, sans-serif'}}>
                      Learn More
                    </a>
                  </Link>
                )}
