...
type SagittariusProduct = {
  ...
  category: string;
  sagittariusBenefit: string;
  slug: string;
};
type SagittariusCategory = {
  name: string;
  benefit: string;
  image: string;
};

function hasBenefit(obj: SagittariusProduct | SagittariusCategory): obj is SagittariusCategory {
  return (obj as SagittariusCategory).benefit !== undefined;
}
...
// Helper to map category id to label (key of sagittariusRecommendations)
function getCategoryLabelById(id: string): keyof typeof sagittariusRecommendations | undefined {
  return categories.find(c => c.id === id)?.label as keyof typeof sagittariusRecommendations | undefined;
}
    ...
    <div className="space-y-12">
      ...
      {activeCategory !== 'specially' && (
        <motion.div
          ...
          transition={{ duration: 0.6 }}
        >
          {(sagittariusRecommendations[getCategoryLabelById(activeCategory) ?? "Gemstone Therapy"] ?? []).map((category, index) => (
            <motion.div
              key={index}
