        ...
        <div className="flex items-center flex-1 min-w-0">
          <div className="flex-shrink-0 mr-10">
            <Link href="/" className="flex items-center gap-3">
              <div className="text-center">
                <h1 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#6A0DAD] to-[#800080] whitespace-nowrap">
                  {t("header.logo.line1")}
                </h1>
                <p className="text-xs text-purple-700 whitespace-nowrap">
                  {t("header.logo.line2")}
                </p>
              </div>
            </Link>
          ...
          <nav className="hidden md:flex items-center gap-5">
            ...
            <Link
              href="/courses"
              className={`font-semibold text-gray-700 hover:text-purple-600 transition-colors ${
                pathname === "/courses" ? "text-purple-600" : ""
              }`}
            >
            ...
            <Link
              href="/blog"
              className={`font-semibold text-gray-700 hover:text-purple-600 transition-colors ${
                pathname?.startsWith("/blog") ? "text-purple-600" : ""
              }`}
            >
              {t("header.nav.blog")}
            </Link>
            <Link
              href="/study"
              className={`font-semibold text-gray-700 hover:text-purple-600 transition-colors ${
                pathname?.startsWith("/study") ? "text-purple-600" : ""
              }`}
            >
              {t("header.nav.study")}
            </Link>
            <div className="relative" ref={servicesMenuRef}>
              <button
                ...
                className="font-semibold text-gray-700 hover:text-purple-600 transition-colors flex items-center gap-1"
              >
                    ...
                    <div className="grid grid-cols-4 gap-x-8">
                      {Object.entries(servicesMegaMenu).map(
                        ([sectionKey, sectionValue]) => (
                          <div key={sectionKey}>
                            <h3 className="text-sm font-bold uppercase text-purple-900 bg-purple-100 rounded px-3 py-1 mb-4 inline-block shadow-sm border border-purple-200">
                              ...
                              {sectionValue.items.map((item) => (
                                <li key={item.key}>
                                  <Link
                                    ...
                                    className="text-base text-gray-700 hover:text-purple-600 font-medium transition-colors"
                                  >
                                    {t(
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          ...
          <div className="hidden md:flex items-center gap-4">
            <a
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-[#FF8C00] to-[#6A0DAD] text-white font-semibold  hover:scale-105 hover:shadow-lg ease-in-out whitespace-nowrap shadow"
            ...
            {session?.user ? (
              <div className="relative" ref={userMenuRef}>
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white text-transparent bg-clip-text bg-gradient-to-r from-[#6A0DAD] to-[#FF8C00] hover:bg-gray-100 transition-all font-semibold whitespace-nowrap"
                >
                  <User className="w-4 h-4" /> {session.user.name || "User"}{" "}
                ...
                {isUserMenuOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                    <div className="py-1">
                      ...
                      >
                        <LogOut className="w-4 h-4" /> {t("header.auth.logout")}
                      </button>
                    </div>
                  </div>
                )}
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white text-transparent bg-clip-text bg-gradient-to-r from-[#6A0DAD] to-[#FF8C00] hover:bg-gray-100 transition-all font-semibold whitespace-nowrap"
              >
                <LogIn className="w-4 h-4" /> {t("header.auth.signin")}
              </button>
            )}
            <div className="relative" ref={langDropdownRef}>
              <button
                onClick={() => setIsLangDropdownOpen((prev) => !prev)}
                className="px-4 py-2 rounded-lg bg-white text-transparent bg-clip-text bg-gradient-to-r from-[#6A0DAD] to-[#FF8C00] hover:bg-gray-100 transition-all font-semibold flex items-center gap-2 whitespace-nowrap"
                  ...
                  <div className="py-2">
                    ...
                    {languageList.map((code) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-sm hover:bg-[#F3E8FF] ${
                          lang === code
        ...
        <DialogContent className="bg-[#FEF9E6]/90 backdrop-blur-lg border-purple-300 border rounded-2xl p-6 max-w-2xl shadow-2xl sm:rounded-3xl">
          <form onSubmit={handleSearchSubmit}>
            <div className="relative">
              ...
              <Button
                type="submit"
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-purple-600 hover:bg-purple-700 text-white font-bold h-10 px-8 text-md"
              >
                Search
              </Button>
            </div>
