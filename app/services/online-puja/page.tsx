...
// Online Puja Service Configuration
const onlinePujaService = {
  ...
  variants: [
    { label: "Basic Puja Package", image: "/images/course-2.jpg" },
    { label: "Complete Puja Ceremony", image: "/images/astrology_understanding.jpg" },
  ],
  features: ["Expert Priests", "Live Streaming", "Personalized Ceremony"],
  price: "₹2,500",
  oldPrice: "₹4,000",
  discount: "37% OFF",
...
// Online Puja FAQs
const onlinePujaFaqs = [
  {
    question: "What is Online Puja and how does it work?",
    answer: "Online Puja is a modern way to perform traditional Hindu ceremonies remotely. Our expert priests conduct the puja live via video streaming, allowing you to participate from anywhere in the world while maintaining the authenticity and spiritual significance of the ceremony.",
  },
  {
    question: "How do I participate in an Online Puja?",
    answer: "You'll receive a secure video link before the ceremony. Simply join the call at the scheduled time, and our priest will guide you through the entire process. You can see, hear, and participate in all the rituals as if you were physically present.",
  },
  {
    question: "What types of Pujas are available online?",
    answer: "We offer various pujas including Grah Shanti, Ganesh Puja, Lakshmi Puja, Saraswati Puja, Rudrabhishek, and many more. Each puja is customized according to your specific needs and astrological requirements.",
  },
  {
    question: "Do I need to prepare anything for the Online Puja?",
    answer: "We'll provide you with a detailed list of items needed for the puja. Most items are common household items, and we can also arrange for special items to be delivered to your location if needed.",
  },
  {
    question: "How long does an Online Puja typically take?",
    answer: "The duration varies depending on the type of puja. Basic pujas take 30-45 minutes, while elaborate ceremonies can take 1-2 hours. We'll inform you of the exact duration when you book.",
  },
  {
    question: "Is Online Puja as effective as physical Puja?",
    answer: "Yes, Online Puja is equally effective when performed with proper devotion and by qualified priests. The spiritual energy transcends physical distance, and our experienced priests ensure all rituals are performed with complete authenticity.",
  },
  {
    question: "Can I record the Puja ceremony?",
    answer: "Yes, you can record the ceremony for your personal use. We also provide a recording of the complete puja that you can keep for future reference and spiritual guidance.",
  },
];
        ...
        <div className="max-w-5xl w-full flex flex-col md:flex-row gap-8">
          ...
          <div className="flex flex-col items-center md:w-1/2">
            <div className="w-full rounded-xl overflow-hidden bg-[#f7f5ed] flex items-center justify-center mb-3" style={{ aspectRatio: '1/1', maxWidth: 340 }}>
              <Image
                src={onlinePujaService.images[selectedImage]}
                alt={onlinePujaService.title}
                width={320}
                height={320}
            ...
            <div className="flex flex-row gap-2 w-full overflow-x-auto pb-2">
              {onlinePujaService.images.map((img, idx) => (
                <button
                  key={img}
                ...
                >
                  <Image src={img} alt={onlinePujaService.title} width={54} height={54} className="object-cover w-full h-full" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col gap-3">
            <h1 className="text-2xl md:text-3xl font-semibold text-[#23244a]" style={{ fontFamily: 'Playfair Display, serif', fontWeight: 500 }}>{onlinePujaService.title}</h1>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[#FFD700] text-lg">&#9733;</span>
              <span className="text-base font-medium text-[#23244a]">{onlinePujaService.rating}</span>
              <span className="text-sm text-[#23244a]">{onlinePujaService.reviews} reviews</span>
            </div>
            <div className="flex gap-2 mt-2">
              {onlinePujaService.features.map((f, i) => (
            ...
            <div className="flex items-end gap-3 mt-3">
              {secondsLeft === 0 ? (
                <span className="text-xl font-bold text-black" style={{ fontFamily: 'Playfair Display, serif', fontWeight: 500 }}>{onlinePujaService.oldPrice}</span>
              ) : (
                <>
                  <span className="text-xl font-bold text-black" style={{ fontFamily: 'Playfair Display, serif', fontWeight: 500 }}>{onlinePujaService.price}</span>
                  <span className="text-base text-gray-400 line-through">{onlinePujaService.oldPrice}</span>
                  <span className="text-base font-semibold text-green-700">{onlinePujaService.discount}</span>
                </>
              )}
            </div>
            ...
            <div className="flex gap-4 mt-3">
              {onlinePujaService.variants.map((v, idx) => (
                <button
                  key={v.label}
            ...
            <div className="text-xs text-gray-600 mt-1">{onlinePujaService.orders.toLocaleString()} orders placed in the last 24 hours</div>
            {/* Delivery Date Input */}
            ...
            <div className="flex gap-3 mt-5">
              <UniversalCartButton
                ...
                productName={onlinePujaService.title}
                price={Number(onlinePujaService.price.replace(/[^\d]/g, ''))}
                image={onlinePujaService.images[0]}
                quantity={quantity}
