...
// Panchang Service Configuration
const panchangService = {
  ...
  variants: [
    { label: "Daily Panchang", image: "/images/cosmiccalendar.png" },
    { label: "Detailed Panchang", image: "/images/astrology_understanding.jpg" },
  ],
  features: ["Daily Auspicious Timings", "Vedic Calendar", "Expert Analysis"],
  price: "₹299",
  oldPrice: "₹599",
  discount: "50% OFF",
  offerEnds: "03 hr : 15 min : 30 sec",
  rating: 4.8,
  reviews: 892,
  orders: 1567,
};
...
const panchangFaqs = [
  {
    ...
    answer: "Panchang is a Vedic almanac that provides daily auspicious timings, planetary positions, and cosmic influences. It helps you choose the best times for important activities and ceremonies.",
  },
  {
    question: "What information does the Panchang provide?",
    answer: "Our Panchang includes Tithi (lunar day), Vara (weekday), Nakshatra (lunar mansion), Yoga (astrological combination), Karana (half of tithi), and Muhurta (auspicious timings).",
  },
  {
    question: "How often is the Panchang updated?",
    answer: "Our Panchang is updated daily with accurate calculations based on your location and current planetary positions.",
  },
  {
    question: "Can I get Panchang for specific dates?",
    answer: "Yes, you can request Panchang for any specific date, including past and future dates for planning important events.",
  },
  {
    question: "Is the Panchang calculation location-specific?",
    answer: "Yes, our Panchang calculations are location-specific to provide the most accurate timings for your area.",
  },
  {
    question: "How do I use Panchang for daily activities?",
    answer: "Check the auspicious timings (Muhurta) for important activities like starting new ventures, ceremonies, or travel. Avoid inauspicious periods for critical decisions.",
  },
  {
    question: "Can I get personalized Panchang recommendations?",
    answer: "Absolutely! Our expert astrologers can provide personalized Panchang guidance based on your birth chart and specific needs.",
  },
];
...
export default function PanchangPage() {
  ...
  const OFFER_DURATION = 3 * 60 * 60 + 15 * 60 + 30; // 3 hr 15 min 30 sec in seconds
        ...
        <div className="max-w-5xl w-full flex flex-col md:flex-row gap-8">
          ...
          <div className="flex flex-col items-center md:w-1/2">
            <div className="w-full rounded-xl overflow-hidden bg-[#f7f5ed] flex items-center justify-center mb-3" style={{ aspectRatio: '1/1', maxWidth: 340 }}>
              <Image
                src={panchangService.images[selectedImage]}
                alt={panchangService.title}
                width={320}
                height={320}
            ...
            <div className="flex flex-row gap-2 w-full overflow-x-auto pb-2">
              {panchangService.images.map((img, idx) => (
                <button
                  key={img}
                ...
                >
                  <Image src={img} alt={panchangService.title} width={54} height={54} className="object-cover w-full h-full" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col gap-3">
            <h1 className="text-2xl md:text-3xl font-semibold text-[#23244a]" style={{ fontFamily: 'Playfair Display, serif', fontWeight: 500 }}>{panchangService.title}</h1>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[#FFD700] text-lg">&#9733;</span>
              <span className="text-base font-medium text-[#23244a]">{panchangService.rating}</span>
              <span className="text-sm text-[#23244a]">{panchangService.reviews} reviews</span>
            </div>
            <div className="flex gap-2 mt-2">
              {panchangService.features.map((f, i) => (
            ...
            <div className="flex items-end gap-3 mt-3">
              {secondsLeft === 0 ? (
                <span className="text-xl font-bold text-black" style={{ fontFamily: 'Playfair Display, serif', fontWeight: 500 }}>{panchangService.oldPrice}</span>
              ) : (
                <>
                  <span className="text-xl font-bold text-black" style={{ fontFamily: 'Playfair Display, serif', fontWeight: 500 }}>{panchangService.price}</span>
                  <span className="text-base text-gray-400 line-through">{panchangService.oldPrice}</span>
                  <span className="text-base font-semibold text-green-700">{panchangService.discount}</span>
                </>
              )}
            </div>
            ...
            <div className="flex gap-4 mt-3">
              {panchangService.variants.map((v, idx) => (
                <button
                  key={v.label}
            ...
            <div className="text-xs text-gray-600 mt-1">{panchangService.orders.toLocaleString()} orders placed in the last 24 hours</div>
            {/* Delivery Date Input */}
            ...
            <div className="flex gap-3 mt-5">
              <UniversalCartButton
                productId="panchang"
                productName={panchangService.title}
                price={Number(panchangService.price.replace(/[^\d]/g, ''))}
                image={panchangService.images[0]}
                quantity={quantity}
